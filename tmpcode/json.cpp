...
static ErrMsg _gErrMsg[] = {
    ...
    { E_UKNOWN_SYNTAX, "Unknown json syntax" },
    ...
    { E_EXTRAL_CHARACTERS, "Unexpected extra characters"}
};
...
void Json::DumpValue(JsonValue* pJV, std::string& strDump, int nSpace) const
{
    if (pJV)
    {
        ...
        case ObjectType:
            if (pJV->_sVal._pJsonVal != NullPtr)
            {
                pJV->_sVal._pJsonVal->DumpValueGroup(
                    pJV->_sVal._pJsonVal->_pGrpVal, strDump,
                    nSpace >0 ? nSpace + FORMAT_SPACE : nSpace);
            }
            else
            {
                strDump += "{}";
        ...
        case ArrayType:
            ...
            {
                DumpArray(pJV->_sVal._pArrVal, strDump,
                        nSpace >0 ? nSpace + FORMAT_SPACE : nSpace);
            }
            else
            {
